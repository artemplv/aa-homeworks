...
def top_titles
  ...
  Movie.select(:title).where('score >= 9.0')
end
...
def below_average_years
  ...
  Movie.select("yr, COUNT(*) AS bad_movies").group(:yr).where('score < 5.0').order('bad_movies DESC')
end
...
def pulp_fiction_actors
  ...
  Movie.joins(:actors).where("movies.title = 'Pulp Fiction'").select("actors.id, actors.name")
end
